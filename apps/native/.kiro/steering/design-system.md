...
- **HeroUI Native** as primary component library
...
1. **Replace Tailwind with HeroUI Components**
   - Use `<Button>`, `<TextField>`, `<Card>`, `<Surface>` instead of raw `<View>` + Tailwind
   - Maintain visual parity but use semantic components

2. **Color Tokens**
   - Map Figma colors to HeroUI theme tokens
   - Use `useThemeColor()` for dynamic colors
   - Prefer semantic names: `foreground`, `background`, `muted`, `danger`, `success`
...
4. **Typography**
   ...
   - Common patterns: `text-4xl font-bold`, `text-base`, `text-sm text-muted`
